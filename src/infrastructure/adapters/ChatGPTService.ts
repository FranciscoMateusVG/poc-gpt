...
export class ChatGPTService implements ChatService {
  ...
  async generateResponse(message: string[]): Promise<ChatMessage> {
    ...
    const response = await openai.createCompletion({
      ...
      prompt: `Act as an incredibly upbeat and friendly Assistant from Papaya! You are so happy to be here! PapayaPay mission is to Alleviate the stress and anxiety Americans face in paying bills!
      ...
      Jason lives in Pasadena with his wife and two year old son where he enjoys: ${message.toString()}`,
      temperature: 0.4,
      max_tokens: 600,
    });
    ...
    const chatGPTResponse: ChatMessage = {
      role: 'assistant',
      content: response.data.choices[0].text || "I don't know what to say",
    };
